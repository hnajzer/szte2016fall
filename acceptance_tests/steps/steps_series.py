...
@given(u'I create a series.py called "{title}" with summary "{summary}" and with {seasons:d} seasons')
@when(u'I create a series.py called "{title}" with summary "{summary}" and with {seasons:d} seasons')
def step_impl(context, title, summary, seasons):
    payload = {'title': title, 'summary': summary, 'seasons': seasons}
    context.response = requests.post(context.url + '/series.py', json=payload)
    context.lastSeriesId = str(context.response.json()['id'])

@when(u'I create a series.py called "{title}"')
...
@when(u'I get this series.py back')
def step_impl(context):
    context.response = requests.get(context.url + '/series.py/' + context.lastSeriesId)
    context.lastSeries = context.response.json()

@when(u'I delete this series.py')
def step_impl(context):
    context.response = requests.delete(context.url + '/series.py/' + context.lastSeriesId)

@when(u'I modify the summary to "{summary}"')
def step_impl(context, summary):
    payload = {'summary': summary}
    context.response = requests.patch(context.url + '/series.py/' + context.lastSeriesId, json=payload)
